...
int main() {
    UI ui;
    Hangman hangman;
    Statistics<string> stats("Hangman");
    ...
    while (again == 'y' || again == 'Y') {
        ...
        try {
            if (difficulty < 1 || difficulty > 3) {
                throw invalid_argument("Neispravan unos za tezinu! Mora biti 1, 2 ili 3.");
            }

            cout << "Odabrana tezina: " << difficulty << endl;
        }
        catch (const invalid_argument& e) {
            cout << "Greska: " << e.what() << endl;
            difficulty = 1; 
            cout << "Postavljena zadana tezina: " << difficulty << endl;
        }
        cin.ignore(10000, '\n');
